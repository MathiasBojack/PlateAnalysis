...
for cnt = 1:CNT
    ...
    dname = ['./3.Thermoelastic/Kirchhoff-Love/DeformedShape/Deformee_a_',...
        num2str(Input.Geometry.a),'_b_',num2str(Input.Geometry.b),'_h_',...
        num2str(Input.Geometry.h),'_Time_',num2str(Input.Load.Time),'_KL.mat'];
    save(dname,'Solution')
    disp(cnt/CNT)
    dep_Max(cnt) = min(Solution.W(:));
end
dep_Max = dep_Max';